# Data Transfer: Moving Files To and From Hyak Klone

Efficient and reliable data transfer is a key part of working on Hyak Klone. There are two main ways to move files between your local computer and the cluster:
...
Replace `UWNetID` with your UW NetID in both the login and the path.
...
Use `scp -r` command to transfer a directory and all its contents.
...
> 
> Globus public sharing is available with [**<ins>Kopah S3 storage</ins>**](https://hyak.uw.edu/docs/storage/gui#globus), which we recommend as a complement for your research storage portfolio if you anticipate regular external sharing and collaboration.
...
This navigation can also be done by typing the path into the Path field, `/mmfs1/gscratch/scrubbed/UWNetID/klone-onboarding-cheme599a` after replacing the word UWNetID with your UW NetID. 
...
- In one panel of the Globus File Manager, open UW Hyak Klone.
- In the other panel, open your personal endpoint (your local computer).

**Try:**

1. Transferring a file from **Klone → Local**. \
   The image below shows the two pane view with Klone on the right and your local collection on the left. Select one of the files from the `klone-onboarding-cheme599a` repository to transfer and click start. You will see a green status box and detailed transfer steps can be viewed. An email will be sent to you when the transfer is completed. 
...
2. Transferring a file from **Local → Klone**. \
   Repeat the same steps but in the opposite direction. **Uncheck the file on the Klone side**, this is required to send files the other way. On the Local side, check the box next to any file and click start to begin the transfer to Klone. 
